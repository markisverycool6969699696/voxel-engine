...
- **User-requested revert (2026-07-22):** after a long same-night session stacking a large feature
  wishlist (crosshair, inventory, menu, save/load, Creative/Survival) plus several rounds of bug
  fixes on top of it (texture-tiling, semantic colors, vertical-section-culling, mesh-rebuild
  throttling, render-distance trims), the user explicitly asked to revert to "when Opus did those
  changes" / "when textures were fine." Targeted the last commit before any of that stack —
  `62eb39c` (Opus's terrain gen + pathfinding, plus the orientation/performance fixes that made the
  user say "GOOD FINALLY" — deliberately *not* the literal last commit Opus made, which still had
  the upside-down bug). Done as a new commit restoring that file state (`git checkout 62eb39c --
  .` + deleting files added after it), not a hard reset — full history intact, nothing force-pushed.
  This means the UI pipeline/crosshair/inventory/menu/save-load/Creative-Survival and every bug fix
  from that session are gone from the working tree as of this revert; they're all still fully
  present in git history (commits `62eb39c..654f7c6`) if any of it needs resurrecting later.

- **Checkpoint (2026-07-22): real GPL v2 textures + a 76-item inventory picker.** User asked to
  build forward from the reverted stable base: find a real (not placeholder) GPL-compatible
  texture pack, add a lot of items, and let the player select what to place from their inventory.
  - **Texture pack**: found and used [SimpleRP](https://github.com/elhedran/SimpleRP) (GPL v2,
    commit `341c49484fc8b7138d6d5ff60f48fcf59fb4bf81`) — downloaded 74 curated 16px PNGs (stone,
    dirt, grass, sand, 5 ore types, 6 wood species × log+planks+leaves, 16 glass colors, 16
    terracotta colors, sandstone, cobblestone, stonebrick, brick, clay, obsidian, cactus). The
    pack supplements vanilla Minecraft rather than replacing it, so it has no water/lava/snow —
    those specific block ids still use a flat procedural color (documented in CREDITS.md and
    `ATLAS_TILES`'s own comments, not silently missing).
  - `engine-core/src/mesh.rs`: replaced the old arbitrary-hash `tile_for_block` with a new
    `ATLAS_TILES: &[(u16, TileSource)]` — an explicit ordered list, `TileSource` is either a real
    PNG filename or a solid-color fallback. This is now the single source of truth both crates
    read for "how many atlas tiles" and "which tile is which block." Also **re-applied two fixes
    from the reverted session** since they're independently correct and matter more with real
    textures than they did with flat placeholder colors: per-block UV tiling (a merged quad's
    texture repeats once per block instead of stretching across the whole face) and the matching
    `mesh.wgsl` `fract()`. Did **not** re-apply the vertical-cross-section-culling fix, the
    mesh-rebuild throttle, or the render-distance trims from the reverted session — those were
    reactive to problems from that session's specific feature stack, out of scope for this ask.
  - `render-vk`: new `image` crate dependency (pure Rust, PNG-only feature, no C deps — none of
    the GNU-toolchain/windows-rs risk the earlier `rodio` crash had). `generate_atlas_pixels`
    rewritten to walk `ATLAS_TILES` in order, decoding real PNGs or drawing the old checkered
    flat-color fallback per tile. New `atlas_textures.rs` embeds all 74 PNGs via `include_bytes!`
    (the 74 entries were **generated by a script reading the actual downloaded filenames**, not
    hand-typed, specifically to avoid transcription errors at that count) — keeps the project's
    established no-runtime-file-dependency pattern (`blocks.json`/`items.json` are `include_str!`'d
    the same way). New public `swatch_color(block_id)` returns a tile's real (or fallback) average
    color, for UI use.
  - `game/data/blocks.json` grew 13 → 78 entries, `items.json` 4 → 76 — the "add a lot of items"
    ask, directly backed by the new texture variety.
  - **Re-added the UI pipeline, crosshair, and inventory picker** (`game/src/ui.rs`,
    `render-vk/shaders/ui.wgsl`, the second no-depth-test alpha-blended pipeline in
    `render-vk/src/lib.rs`) — these were exactly what the revert removed, needed again for
    "select what blocks to put in your inventory." `E` opens a grid of every placeable block
    (swatch colored via `swatch_color`, so what you see matches what you place); click one to
    select it. **Grid redesigned for the new item count**: 10 columns (was 5, sized for ~9 items)
    so 76 items fit in ~8 rows without overflowing NDC's `-1..1` range — verified with a dedicated
    test (`full_76_item_grid_fits_within_ndc_bounds`), not just visually assumed. Deliberately
    **did not** re-add the main menu, save/load, or Creative/Survival distinction from the reverted
    session — kept this batch scoped to exactly what was asked (textures + inventory), not a
    silent full re-application of everything that got reverted.
  - Verified: `cargo test --workspace` 87/87 (was 81; +4 `ui.rs` tests, +2 `mesh.rs` tests for the
    new `ATLAS_TILES` lookup, net of a couple removed with the old hash-based tile assignment),
    clean build zero warnings, 20s soak clean, no leftover process. Visual confirmation (do the
    textures actually look right, is the inventory grid usable at 76 items) is the user's to give.

## Next Up
**Both Opus-tier milestones are done** (terrain generation + biome blending, and pathfinding
around partially-loaded chunks).

What's left, roughly in order of what the user has been asking for recently:
- Fluid behavior for water (currently a solid walkable block, still a flat procedural color since
  the texture pack has no water equivalent; `BehaviorTag::Fluid` already exists in the registry as
  the hook) — Sonnet-tier.
- If the user wants the reverted session's other features back (main menu, save/load, Creative/
  Survival, the vertical-cross-section-culling fix, mesh-rebuild throttling) — they're all in git
  history (`62eb39c..654f7c6`), not lost, just deliberately left out of this batch. Worth asking
  before silently re-adding any of it, given the revert was an explicit, considered choice.
- Per-chunk GPU mesh buffers (perf; the current single-combined-buffer design is what made the
  mesh-rebuild lag possible in the reverted session at wider render distances — relevant again if
  render distance is ever increased past the current default of 4).
- Terrain/nav tuning, Metal/macOS backend, multiplayer — unchanged, still future work.
